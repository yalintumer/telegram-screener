...
def stochastic_rsi(close: pd.Series, rsi_period=14, stoch_period=14, k=3, d=3):
...
def stoch_rsi_buy(df: pd.DataFrame):
    if len(df) < 2: return False
    prev, last = df.iloc[-2], df.iloc[-1]
    if any(pd.isna(v) for v in [prev.k, prev.d, last.k, last.d]): return False
