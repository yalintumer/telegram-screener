...
class TestCheckMarketFilter:
    ...
    def test_fails_on_low_market_cap(self, mock_price_data):
        ...
        with patch('src.filters.daily_ohlc', return_value=mock_price_data), \
...
class TestCheckWavetrendSignal:
    ...
    def test_multi_timeframe_disabled_skips_weekly(self):
        ...
        with patch('src.filters.daily_ohlc', return_value=mock_df), \
            ...
            mock_wt.return_value = pd.DataFrame({
                'wt1': [-60.0] * 50,
                'wt2': [-65.0] * 50
            })
